...
def get_train_transforms_step1():
...
######### Step3 Transformations Start ###########

def get_train_transforms_step3():
  ...
  return v2.Compose([
    # Randomly apply CenterCrop(22) with a 15% probability
    # Purpose: Introduce variability during training by randomly cropping images.
    v2.RandomApply([v2.CenterCrop(22), ], p=0.15),


    # Randomly rotate the image by an angle between -6 and +12 degrees
    # fill = 0 to fill with black pixels as background
    # Purpose: Augment the training data by introducing random rotations.
    v2.RandomRotation((-6., 12.), fill=0),
    

    # Randomly add distortion in images with 15% probability 
    v2.RandomPerspective(distortion_scale=0.3, p= 0.15),
...
def get_MNISTdataset_with_loader(datasettype,stepnumber,kwargs):

  '''
  <datasettype> can have values as 'train' or 'test'
  <stepnumber> can have values as 1 or 3
  ...
  if stepnumber==1 or stepnumber==2:
    if datasettype == 'train':
      train_data = datasets.MNIST('../data', train=True, download=True, transform=get_train_transforms_step1())  # download and load the "training" data of MNIST and apply train_transform
      print("Training data loaded successfully...")
      return torch.utils.data.DataLoader(train_data, **kwargs) # load train data
    elif datasettype == 'test':
      test_data = datasets.MNIST('../data', train=False, download=True, transform=get_test_transforms_step1())   # download and load the "test" data of MNIST and apply test_transform
    ...
    if datasettype == 'train':
      train_data = datasets.MNIST('../data', train=True, download=True, transform=get_train_transforms_step3())  # download and load the "training" data of MNIST and apply train_transform
      print("Training data loaded successfully...")
      return torch.utils.data.DataLoader(train_data, **kwargs) # load train data
    elif datasettype == 'test':
      test_data = datasets.MNIST('../data', train=False, download=True, transform=get_test_transforms_step3())   # download and load the "test" data of MNIST and apply test_transform
      print("Testing data loaded successfully...")
      return torch.utils.data.DataLoader(test_data, **kwargs) # load test data 
    else:
      raise ValueError('Incorrect dataset type string...pass valid name from available values')
      ...
      raise ValueError('Incorrect dataset type value...pass valid number from available values')
