...
def get_train_transforms():
  ...
  return v2.Compose([
    ...
    v2.Normalize(mean=mean_list, std = std_list),
    ])
...
def get_CIFARdataset_with_loader(datasettype,kwargs):
    ...
    if datasettype == 'train':
        train_data = datasets.CIFAR10('../data', train=True, download=True, transform=get_train_transforms())  # download and load the "training" data of CIFAR and apply train_transform
        print("Training data loaded successfully. Shape of data: ",train_data.data.shape)
        return train_data.class_to_idx, torch.utils.data.DataLoader(train_data, **kwargs)    # load train data
    elif datasettype == 'test':
        test_data = datasets.CIFAR10('../data', train=False, download=True, transform=get_test_transforms())   # download and load the "test" data of CIFAR and apply test_transform
        print("Testing data loaded successfully. Shape of data: ",test_data.data.shape)
        return test_data.class_to_idx, torch.utils.data.DataLoader(test_data, **kwargs)      # load test data
    else:
...
def post_accuracyplots(train_losses,test_losses,train_acc,test_acc,normtype):
  ...
  fig.suptitle(f"Plots - {normtype}", fontsize=16)
